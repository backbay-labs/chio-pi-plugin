...
The supported SDK host process is trusted and is not OS-sandboxed. It can read its prepared credential and contact loopback. Model-reachable native tools and resource loading are disabled, and the protected filesystem is in a separate resource container. Arbitrary process compromise or hostile same-UID code is outside the demonstrated boundary. Session-scoped credentials can restrict kernel authority but cannot by themselves prove OS process isolation.
