...
async function main() {
  ...
  if (args.length === 1 && args[0] === "--help") {
    process.stdout.write("Usage: chio-pi --config /absolute/delegated.json --profile /absolute/profile --cwd /absolute/disposable-workspace --provider openai --model gpt-4.1-mini --prompt 'task' [--resume /absolute/profile/sessions/session.jsonl]\nProtected candidate requires macOS, an installed package, and delegated retained-session credentials.\n");
    return;
  }
  ...
  if (values.get("--provider") !== "openai" || values.get("--model") !== "gpt-4.1-mini") throw new Error("Qualified model relay supports only openai/gpt-4.1-mini");
